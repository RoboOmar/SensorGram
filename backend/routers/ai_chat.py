...
def ask_expert(req: AIChatRequest, current_user: Robot = Depends(get_current_robot)):
    if not model:
        return {"response": f"Hello {current_user.display_name}, your AI engine is ready. Please add your GEMINI_API_KEY to the configuration to activate my neural network."}
